resource "kubernetes_deployment" "this" {
  for_each = { for msa_config in var.src.msa_config : msa_config.identify => msa_config }

  metadata {
    name      = local.msa_config[each.key].metadata.name
    namespace = local.msa_config[each.key].metadata.namespace
    labels    = local.msa_config[each.key].metadata.labels
  }

  spec {
    replicas = local.msa_config[each.key].spec.replicas

    selector {
      match_labels = local.msa_config[each.key].spec.selector.match_labels
    }

    strategy {
      type = local.msa_config[each.key].spec.strategy.type
      rolling_update {
        max_surge       = local.msa_config[each.key].spec.strategy.rolling_update.max_surge
        max_unavailable = local.msa_config[each.key].spec.strategy.rolling_update.max_unavailable
      }
    }
    ...
    template {
      ...
      spec {
        dynamic "toleration" {
          for_each = local.msa_config[each.key].spec.tolerations
        ...
        dynamic "affinity" {
          for_each = local.msa_config[each.key].spec.affinity
        ...
        dynamic "container" {
          for_each = local.msa_config[each.key].spec.containers
